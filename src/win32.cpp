...
#define USE_NEW_INTERACTIVE_DESKTOP
		...
		BOOL FillProcessesListWithAlloc(DWORD *pprocesses, DWORD size, DWORD *pnbProcesses)
		{
		...
		DWORD FillProcessesList(DWORD *pprocesses, DWORD bufsize)
		{
		...
		BOOL ProcessIdToName(DWORD processId, TCHAR* processName, DWORD buffSize)
		{
			...
			if (handle){
				...
				if (copied > 0 && copied <= buffSize){
					...
					printf("Error QueryFullProcessImageName for %d :0x%08X %d", processId, GetLastError(), GetLastError());
				}
#endif
				CloseHandle(handle);
			}
